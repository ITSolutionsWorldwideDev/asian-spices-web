...
import {
  ...
  RotateCcw,
  ShieldCheck,
  AlertCircle, // 🟢 Added for notice icon
} from "lucide-react";
...
export default function ProductDesc({ product }: { product: any }) {
  ...
  // 🟢 Detect if price is unavailable for this country context
  const isPriceAvailable = product.min_offered_price !== undefined && product.min_offered_price !== null;

  // Fallback calculations safely guarded
  const rawBasePrice = Number(product.min_offered_price || product.base_price || 0);
  const currentPrice = product.sale_price ? Number(product.sale_price) : rawBasePrice;
  ...
  if (isPriceAvailable && originalPrice && originalPrice > currentPrice) {
    ...
    if (product.discount_type === "percentage" || product.discount_type === "Bulk") {
      activeBadge = product.discount_value && !isNaN(discountNum)
  ...
  const features = [{ icon: Truck, title: "Free Shipping on €50+" }, { icon: RotateCcw, title: "30- Day Returns" }, { icon: ShieldCheck, title: "Quality Guarantee" }];
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mb-6 text-sm sm:text-base">
        ...
        <span className="text-[#6A7282]">/</span>
        <p className="text-gray-900 font-medium wrap-break-word">{product.name}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-6">
          ...
          <p className="text-gray-500">Origin: {product.country_of_origin || "International"}</p>
            ...
            <div className="space-y-4">
              <div className="flex items-baseline gap-4">
                <span className="text-4xl font-bold text-orange-500">
                  {symbol}{(currentPrice * rate).toFixed(2)}
                </span>
              ...
              {rawSave > 0 && (
                <p className="text-green-600 font-semibold bg-green-50 text-sm py-1 px-3 rounded-lg w-fit border border-green-100">
                  You save {symbol}{(rawSave * rate).toFixed(2)}
                </p>
              )}
            </div>
          ) : (
          ...
          >
            <Heart className={`w-5 h-5 transition-colors ${isInWishlist(product.id) ? "fill-white text-white" : ""}`} />
              ...
              {features.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-gray-600 text-xs font-medium">{item.title}</p>
                  </div>
                );
              })}
            </div>
